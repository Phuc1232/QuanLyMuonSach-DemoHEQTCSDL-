    ...
    public class FrmConcurrencyDemo : Form
    {
        ...
        private void RefreshDatabaseInspector()
        {
            ...
            if (scenario == 0 || scenario == 1)
                query = "SELECT MaCuonSach, TrangThai, TinhTrang FROM CuonSach WHERE MaCuonSach = 'CS001'";
            else if (scenario == 2)
                query = "SELECT MaCuonSach, TenSach, TrangThai FROM CuonSach c JOIN Sach s ON c.MaSach = s.MaSach WHERE c.MaSach = 'S001'";
            else if (scenario == 3)
                query = "SELECT MaPhieuPhat, MaPhieuMuon, LyDoPhat, SoTienPhat FROM PhieuPhat";
